...
Registration imports are recognized for these framework entrypoints:

| Workflow | Supported modules and forms |
| --- | --- |
| Cucumber.js | `@cucumber/cucumber` and the legacy `cucumber` package; ESM, CJS, aliases, namespaces, and local static re-exports. |
| Playwright-BDD | `createBdd()` registrations from `playwright-bdd`, plus class-method `Given`/`When`/`Then`/`Step` decorators from `playwright-bdd/decorators`. |
| Cypress Cucumber | `@badeball/cypress-cucumber-preprocessor`, plus the legacy `cypress-cucumber-preprocessor/steps` registration entrypoint. |

Package entrypoints are matched exactly. In particular, the legacy Cypress package root is not a
step-registration module; use its `/steps` entrypoint. Plain Playwright suites are supported when
...
Use `--explain-discovery` to print the effective pattern origin, selected parser, matching pattern, and definition inputs without changing report output. Unmatched feature patterns are warnings. A definition file warns when its AST contains unresolved call sites shaped like known step registrations; ordinary helper calls do not. CLI-generated machine reports include a deterministic `corpus` census with the number of discovered definition files, files yielding definitions, extracted definitions, discovered feature files, successfully parsed feature files, and converted Gherkin Markdown files that yielded no concrete steps. Set `requireDefinitions: true` or pass `--require-definitions` to make an empty extracted definition corpus an operational failure (exit code `2`).

If definitions exist but no feature files match, CukeDedup warns and disables `unused-definition` findings rather than presenting an incomplete corpus as proof that every definition is unused. A converted Gherkin Markdown file that parses successfully but produces no concrete steps is also named in a warning and marks `corpus.incomplete`; this catches converters that accept a document while omitting unrecognized markup or dialect content. `unused-definition` remains disabled because that partial feature corpus cannot prove non-use. A valid classic `.feature` placeholder with no steps remains a complete input and does not disable usage analysis. Set `requireFeatures: true` or pass `--require-features` to make a missing feature corpus an operational failure (exit code `2`). A module specifier that cannot be resolved statically — one outside the analysis root, or beyond a resolution limit — is reported the same way: a source-localized warning naming the cause, and the run is marked incomplete rather than failed. A module the filesystem refuses to read stays an operational failure, because the analyzer could not read input it was told to read.
