...
- CommonJS `module.exports = { X }`, `module.exports = require('./a')`,
  `module.exports = { ...require('./a'), X }`, `Object.assign(module.exports, require('./a'))`, and
  `exports.X = require('./a').X` (or a local binding).

A CommonJS `module.exports`/`exports.X` assignment written in a form the analyzer does not model —
a factory call, an aliased object, an unresolvable spread — marks the corpus incomplete rather than
resolving silently to nothing, so a barrel that might hide registrations is reported instead of
passing as clean.
